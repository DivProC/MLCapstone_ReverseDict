...
CHECKPOINT  = RESULTS_DIR / "bilstm_attn_best.pt"
...
def run_eval():
    ...
    encoder = BiLSTMEncoder(
        ...
        attn_dim=cfg.get("attn_dim", 128), num_heads=cfg.get("num_heads", 4),
    ).to(device)
    ...
    pd.DataFrame(rows).to_csv(RESULTS_DIR / "bilstm_attn_metrics.csv", index=False)
    pd.DataFrame(top_preds).to_csv(RESULTS_DIR / "bilstm_attn_sample_predictions.csv", index=False)
